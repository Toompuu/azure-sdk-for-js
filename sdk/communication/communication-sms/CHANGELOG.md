...
- `SendOptions` renamed to `SmsSendOptions` and now has an additional field `tag` to add a custom tag to delivery reports (when enabled). Added a field `validityPeriodSeconds` to define maximum delivery report waiting time, its default is 18000 (when delivery report is enabled).
